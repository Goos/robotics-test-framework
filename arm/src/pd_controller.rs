//! Per-joint PD velocity controller — the one piece of genuinely reusable
//! controller infrastructure in the arm crate (design v2 §6, Phase 5 happy
//! path). Scenarios with a fixed task structure (PickPlace, FindGraspPlace,
//! …) compose their own state machines; this module stays unconditionally
//! exported because it's general enough to apply across scenarios.
...
/// Per-joint PD velocity controller. Generic over `R: PortReader<JointEncoderReading>`
/// so it accepts either raw `PortRx<_>` or fault-wrapped readers
/// (Phase 9 fault wrappers) without changes.
///
/// One sensor port and one actuator port per joint, indexed by `i: usize` into
/// both `Vec`s — keeps lookup O(1) and avoids any HashMap dependency.
