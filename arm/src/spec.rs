...
#[derive(Clone, Debug)]
pub enum JointSpec {
    Revolute { axis: Unit<Vector3<f32>>, limits: (f32, f32) },
    Prismatic { axis: Unit<Vector3<f32>>, limits: (f32, f32) },
}
    ...
    fn arm_spec_with_two_revolute_joints() {
        let spec = ArmSpec {
            joints: vec![
                ...
                JointSpec::Revolute { axis: Vector3::y_axis(), limits: (-FRAC_PI_2, FRAC_PI_2) },
            ],
